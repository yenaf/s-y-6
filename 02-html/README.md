...
### 7. 시맨틱 태그
> 웹 페이지를이루는요소에 의미에 맞는 태그
- #### `<header>` : 소개 및 탐색에 도움을 주는 콘텐츠. 제목, 로고, 검색 폼 등의 요소
- #### `<footer>` : 하단에 작성자, 저작권정보, 관련문서 등의 내용을 담는 요소
- #### `<nav>` : 문서의 부분 중 현재페이지 내, 또는 다른 페이지 링크를 보여주는 내비게이션 요소. 메뉴, 목차, 색인등
- #### `<main>` : 주요콘텐츠. 문서에 하나만 존재야함. 문서에서 유일한 내용이며 반복되지 않은 콘텐츠
- #### `<section>` : 독립적인 구획을 나타내며, 공동된 내용을 담고 있는 부분을 묶어주는 요소
- #### `<aside>` : 좌측, 우측 사이드 위치의 공간에 주요콘텐츠 외의 참고가 될수있는 부가 콘텐츠 표시. 사이드바, 광고
- #### `<article>` : 독립적으로 구분되며 재사용할수있는 요소. 게시판, 블로그글, 뉴스기사 등