...
select * from customer where custname like '_지%'; -- 무조건 두글자인데 마지막글자가 지 인것
...
-- 6. order by : 정렬
select * from customer order by custname; -- default asc 정방향정렬
select * from customer order by custname desc; -- 역순정렬
