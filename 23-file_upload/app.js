...
// multer객체.array('인풋name') : 여러 파일을 하나의 인풋에 업로드
app.post('/upload/array',uploadDetail.array('userfiles'), (req,res)=>{
  console.log(req.body);
  console.log(req.files);//배열로 출력
  res.send('Success upload! (mutiple)');
})

// multer객체.fields([{},{},..]) : 여러파일을 각각의 인풋에 업로드 = 인풋여러개
app.post('/upload/fields',uploadDetail.fields([{name : 'kiwi'},{name : 'orange'}]),(req,res)=>{
  console.log(req.body);
  console.log(req.files);//객체로 출력 //{kiwi : [{},...],orange : [{},...]}
  res.send('Success upload! (mutiple2)');
})
